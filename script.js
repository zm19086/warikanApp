...
const create_form = () => {
  ...
  let payment_average = Number((price.value - (price.value % number.value)) / number.value);
  ...
  for (let i = 1; i - 1 < number.value; i++) {
    ...
    let member_element = document.getElementById('member');
    let new_member = document.createElement('li');
    new_member.textContent = 'メンバー' + i + '：';

    let member_inp = document.createElement('input');
    ...
    let member_pay = document.createElement('input');
  ...
  let result_btn = document.createElement('input');
